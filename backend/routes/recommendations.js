...
// ML Service has been removed - using SQL-based recommendations only
// This provides stable, fast recommendations without external service dependencies
...
router.get('/user/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    // SQL-based genre recommendations (built-in, no external service needed)
    if (books.length === 0) {
      try {
...
router.get('/similar/:bookId', async (req, res) => {
  try {
    ...
    // Get books from same country/author with matching genres
    ...
    // First try genre-based similarity
    const [similarBooks] = await db.execute(`
      SELECT b.*, c.name as country_name,
             COUNT(DISTINCT bgr2.genre_id) as matching_genres
      FROM books b
...
// Get trending books (from database, not ML)
router.get('/trending', async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 10;

    console.log(`📈 Getting trending books (limit ${limit})`);

    // Get highest rated books from database
    ...
    res.json({
      trending_books: books.map(book => ({
        ...
        author: book.author,
        rating: book.average_rating,
        recommendation_type: 'sql_trending'
      })),
      total: books.length
    });
  } catch (error) {
...
// Get genre-based personalized recommendations (SQL-based, no ML service dependency)
